...
function Player:render()
	...
	love.graphics.draw(
		...
		self.currentAnimation.quads[spriteIndex],
		self.x,
		self.y,
		0,
		self.scale
...
function Player:update(dt)
	...
	elseif self.direction == "up" then
		self.currentAnimation = self.idleUp
	elseif self.direction == "right" then
		self.currentAnimation = self.idleRight
	else
		self.currentAnimation = self.idleLeft
	end
	...
	if love.keyboard.isDown("w") or love.keyboard.isDown("up") then
		...
		self.direction = "up"
		if self.state == "stop-bottom" then
			self.dy = 0
		end
	...
	if length > 0 then
		self.dx, self.dy = (self.dx / length) * config.PLAYER_SPEED, (self.dy / length) * config.PLAYER_SPEED
	end
